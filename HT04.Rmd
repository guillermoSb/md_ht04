...
abs_error_3 <- abs(mean(y_pred_2 - test_set$salePrice))
# Modelo 4
...
## 6 - Crear una variable respuesta donde podamos clasificar en (Economias, Intermedias o Caras) en base al analisis exploratorio elaborado.

Limites y rangos establecidos para filtrar la economia de las casas:
- Economicas: precio < $150000
- Intermedias: precio >= $150000 and precio <= $400000
- Caras: precio > $400000

Referencias para decidir el rango del precio:
-> https://www.realtor.com/realestateandhomes-search/Iowa-State-University_Ames_IA
-> https://www.redfin.com/neighborhood/759899/IA/Ames/Iowa-State-University/housing-market

```{r}
#economy_var_answer <- ifelse(data$salePrice < 150000, "Económicas", ifelse(data$salePrice >= 150000 & data$salePrice <= 400000, "Intermedias", "Caras"))
train_set$economy <- ifelse(train_set$salePrice < 150000, "Económicas", ifelse(train_set$salePrice >= 150000 & train_set$salePrice <= 400000, "Intermedias", "Caras"))
test_set$economy <- ifelse(test_set$salePrice < 150000, "Económicas", ifelse(test_set$salePrice >= 150000 & test_set$salePrice <= 400000, "Intermedias", "Caras"))
```


## 7 - Arbol de clasificación utilizando la variable respuesta creada en el punto 6

```{r}
arbol_modelo <- rpart(economy~., train_set, method="class")
# Mostrar el modelo gráficamente
rpart.plot(arbol_modelo)
```

## 8 - Utilizar modelo con el conjunto de prueba y determinar eficiencia del algoritmo
...
ypred <- predict(arbol_modelo, newdata = test)
ypred <- apply(ypred, 1, function(x) colnames(ypred)[which.max(x)])
ypred <- factor(ypred)
```

## 9 - Analisis de eficiencia del algoritmo usando una matriz de confusion
